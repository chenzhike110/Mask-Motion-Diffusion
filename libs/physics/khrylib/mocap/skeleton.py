...
from libs.physics.khrylib.utils.transformation import euler_matrix
        ...
        if self.template_geoms is None or len(self.template_geoms[bone.name]) == 0:
            ...
            if g_attr["type"] == "capsule":
                g_attr[
                    ...
                    *np.concatenate(
                        [template_attributes["start"], template_attributes["end"]]
                    )
                )
                ...
                g_attr["pos"] = "{0:.4f} {1:.4f} {2:.4f}".format(*(e1 + e2) / 2)
