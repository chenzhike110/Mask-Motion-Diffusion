...
import math
import numpy as np
from libs.physics.utils.transformation import quaternion_from_matrix
...
class Skeleton:
    ...
    def write_xml(
        ...
        if bump_buffer:
            SubElement(tree.getroot(), "size", {"njmax": "1000", "nconmax": "200"})
        # create sensors
    ...
    def construct_tree(
        ...
        for bone, joint in zip(self.bones[1:], joints[1:]):
    ...
    def write_xml_bodynode(self, bone, parent_node, offset, ref_angles):
        ...
        if os.path.exists(geom_path):
            ...
            #     g_attr["frictionloss"] = "5"
                # g_attr["friction"] = "0.000000005 0.005 0.0001"
                # g_attr["solimp"] = "0.9 0.95 0.001 0.5 2"
                # g_attr["solref"] = "0.02 1"
                # g_attr["gap"] = "0.02 "
