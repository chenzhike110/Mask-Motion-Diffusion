...
from libs.physics.khrylib.rl.envs.common.mjviewer import save_video
from libs.physics.khrylib.rl.utils.visualizer import Visualizer
from libs.physics.utils.transformation import quaternion_twovec, random_quaternion
import mujoco_py


class CopycatVisualizer(Visualizer):

    def __init__(self, vis_file, agent, record=False):
        self.agent = agent
        ...
        if record:
            self.env_vis.viewer._record_video = True
            fps = 20
            self._video_process = Process(
                target=save_video,
                args=(
                    self.env_vis.viewer._video_queue, 
                    self.env_vis.viewer._video_path % self.env_vis.viewer._video_idx, fps),
            )
            self._video_process.start()
            
    def end_record(self):
        self.env_vis.viewer._video_queue.put(None)
        self._video_process.join()
        self.env_vis.viewer._video_idx += 1
    ...
    def update_pose(self):
        ...
        # self.env_vis.model.body_pos[len(self.env_vis.model.body_names)-1] = np.array([0.5, 0., 0.])
        self.env_vis.model.body_quat[self.env_vis.model.body_names.index("chairblack")] = np.array([0.5, 0.5, 0.5, 0.5])
