import torch
import numpy as np
from libs.physics.utils.transformation import quaternion_matrix
...
def compute_metrics(res, converter=None):
    ...
    vel_dist = compute_error_vel(jpos_pred, jpos_gt).mean(axis = -1) * 1000
    accel_dist = compute_error_accel(jpos_pred, jpos_gt).mean(axis = -1) * 1000
...
def compute_skate(vert, info):
    ...
    for t in range(vert.shape[0] - 1):
        cind = (vert[t, :, 2] <= info["floor_z"]) & (vert[t + 1, :, 2] <=
                                                     info["floor_z"])
        if torch.any(cind):
