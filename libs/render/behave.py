...
class MeshRendererWrapper:
    ...
    def __init__(self, image_size=1200,
                 ...
                 materials=None, max_faces_per_bin=200000):
...
class Pyt3DWrapper:
    ...
    def prepare_render(self, meshes, colors):
        ...
        for mesh, color in zip(meshes, colors):
            ...
            py3d_mesh = Meshes([torch.from_numpy(mesh['v']).float().to(self.device)], [torch.from_numpy(mesh['f']).long().to(self.device)],
                               text)
